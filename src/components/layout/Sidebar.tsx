...
import { calculateCombatStats, getCriticalChanceExplanation, getCriticalDamageExplanation } from '../../utils/combat';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className={`${isCompact ? 'w-20' : 'w-64'} ${themeClasses.sidebarBg} h-screen fixed left-0 top-0 z-10 ${themeClasses.shadow} transition-all duration-300 flex flex-col`}>
      {/* Logo */}
          ...
          <div className="text-center space-y-3">
            <h1 className="font-orbitron font-bold text-lg sm:text-xl leading-tight tracking-wide">
              <div className="flex justify-center items-center space-x-1">
                <span className="text-transparent bg-gradient-to-r from-orange-400 to-orange-600 bg-clip-text">URBAN</span>
                <span className="text-transparent bg-gradient-to-r from-blue-400 to-blue-600 bg-clip-text">CLASH</span>
                <span className="text-transparent bg-gradient-to-r from-purple-400 to-purple-600 bg-clip-text">TEAM</span>
              </div>
            </h1>
            {/* Ícones de navegação */}
      ...
      <div className={`mx-2 mb-2 p-2 ${themeClasses.cardBg} rounded-lg ${themeClasses.shadow}`}>
        {isCompact ? (
          <div className="text-center space-y-2">
            <div className="space-y-1 text-xs">
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-green-400">NV</span>
                <span className="font-bold">{formatValue(userProfile?.level || 1)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-purple-400">XP</span>
                <span className="font-bold">{formatValue(userProfile?.current_xp || 0)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-orange-400">EN</span>
                <span className="font-bold">{formatValue(userProfile?.energy || 100)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-green-400">AP</span>
                <span className="font-bold">{formatValue(userProfile?.action_points || 10)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-yellow-400">$</span>
                <span className="font-bold">{formatValue(userProfile?.money || 0)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-red-400">ATK</span>
                <span className="font-bold">{formatValue(combatStats?.attack || userProfile?.attack || 0)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-blue-400">DEF</span>
                <span className="font-bold">{formatValue(combatStats?.defense || userProfile?.defense || 0)}</span>
              </div>
              <div className={`${themeClasses.text} flex justify-between`}>
                <span className="text-purple-400">FOC</span>
                <span className="font-bold">{formatValue(combatStats?.focus || userProfile?.focus || 0)}</span>
              </div>
              <div 
              ...
              >
                ...
                <span className="font-bold">{formatValue(combatStats?.criticalChance || userProfile?.critical_chance || 0)}%</span>
              </div>
              <div 
                className={`${themeClasses.text} flex justify-between cursor-help hover:bg-gray-700/30 rounded transition-colors`}
              ...
              >
                ...
                <span className="font-bold">{formatValue(combatStats?.criticalDamage || 0)}</span>
              </div>
            </div>
          </div>
        ) : (
      ...
      {/* Navigation Items */}
      <nav className={`${isCompact ? 'px-2' : 'px-4'} flex-1 overflow-y-auto custom-scrollbar pb-4`}>
              ...
              <li className="mt-4">
                <div className="space-y-1">
                   ...
                   {gameMenuItems.map((item, index) => {
                     ...
                     const hoverColor = isGangster ? 'hover:bg-orange-500' : 'hover:bg-blue-500';
                     const activeColor = isGangster ? 'bg-orange-500' : 'bg-blue-500';
                     const ringColor = isGangster ? 'ring-orange-400' : 'ring-blue-400';
                     ...
                     return (
                       <button
                         ...
                         className={`w-full ${themeClasses.text} ${hoverColor} hover:bg-opacity-30 font-orbitron py-2 px-2 rounded transition-colors text-center text-sm ${
                           currentPath === item.path ? `${activeColor} bg-opacity-30 ring-2 ${ringColor}` : ''
                         }`}
