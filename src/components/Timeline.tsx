...
const Timeline: React.FC<TimelineProps> = ({ className = '' }) => {
  ...
  const timelineData = [
    ...
    },
    {
      ...
      description: "Gangsters, Guardas ou Clãs - cada um com suas características",
      icon: "⚔️",
  ...
  useEffect(() => {
    const handleMouseMove = () => {
      ...
      if (window.scrollY > 100) { // Só esconde se não estiver no topo
          ...
          {timelineData.map((event, index) => (
            ...
            >
              ...
              {index < timelineData.length - 1 && (
                <div className={`absolute left-5 top-10 w-0.5 h-4 transform -translate-x-1/2 transition-all duration-500 ${
                  index === activeSection 
