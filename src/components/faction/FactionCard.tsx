...
export default function FactionCard({ faction, selectedFaction, onSelect }: FactionCardProps) {
  const isGangsters = faction === "gangsters";
  ...
  const config = {
    gangsters: {
      title: "GANGSTERS",
      color: "orange",
      description: "Domine as ruas com estratégia e força. Os Gangsters controlam o submundo.",
    ...
    guardas: {
      title: "GUARDAS",
      color: "blue",
      description: "Mantenha a ordem e proteja a cidade. Os Guardas defendem a lei e combatem o crime.",
  ...
  return (
    ...
    >
      <div className={`absolute inset-0 bg-gradient-to-br ${colors.gradient} to-black z-0`}></div>

      <div className="relative z-10 p-4 sm:p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className={`text-xl sm:text-2xl md:text-3xl font-orbitron ${colors.title}`}>
            {factionConfig.title}
          </h2>
          <div className={`w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full ${colors.iconBg} flex items-center justify-center`}>
            <svg
        ...
        <div className="flex items-center justify-between">
          <div className="flex space-x-1">
            ...
            <span className={`ml-2 ${colors.powerText} text-sm`}>{factionConfig.powerLabel}</span>
          </div>
