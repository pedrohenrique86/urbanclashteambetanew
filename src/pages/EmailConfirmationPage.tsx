  ...
  useEffect(() => {
    ...
    const confirmEmail = async () => {
      ...
      if (!token || token.length < 10) {
        console.log('❌ Token inválido ou não encontrado, redirecionando para home');
      ...
      try {
        ...
        // Armazenar o token para login automático
        if (response.token) {
          console.log('🔑 Token recebido, fazendo login automático');
        ...
        showToast('Erro ao confirmar email. Por favor, tente novamente.', 'error');
        navigate('/');
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900">
      {isLoading && (
        <div className="bg-gray-800 p-8 rounded-lg shadow-lg max-w-md w-full text-center">
          ...
          <p className="text-gray-300">Aguarde enquanto confirmamos seu email...</p>
        </div>
      )}
    </div>
  );
