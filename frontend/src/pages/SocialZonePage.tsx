...
export default function SocialZonePage() {
  ...
  useEffect(() => {
    ...
    const handleConnect = () => {
      ...
      if (t) socketService.authenticateGlobal(t);
    };

    socketService.onGlobalHistory(handleHistory);
    socketService.onGlobalMessageReceived(handleMessage);
    socketService.onGlobalUsers(handleUsers);
    socketService.on("connect", handleConnect);

    // Autenticação inicial com pequeno delay para garantir que o socket está pronto
    ...
    return () => {
      clearTimeout(timer);
      socketService.off("global:history", handleHistory);
      socketService.off("global:message", handleMessage);
      socketService.off("global:users", handleUsers);
  ...
  }, [messages, activeTab]); // Rola ao mudar de aba também
    ...
    <div className="min-h-screen p-4 md:p-8 bg-transparent relative text-slate-300 font-sans selection:bg-violet-500/30">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div className="flex flex-col gap-3 mt-4">
            <div className="flex items-center gap-4">
              ...
              <span className="text-[10px] font-mono text-violet-400/80 animate-pulse tracking-widest font-bold uppercase hidden md:inline-block">● Uplink_Established</span>
            </div>
            
            <p className="text-slate-300 text-[9px] md:text-[10px] font-mono tracking-[0.2em] uppercase bg-white/5 py-1 px-3 border-l-2 border-violet-500/50 w-fit backdrop-blur-sm">
              {subtitle}
            </p>
          </div>
        </motion.div>
      ...
      <main className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col md:flex-row h-[60vh] md:h-[650px] cyber-card cyber-card-violet bg-black/60 border-violet-500/20 relative overflow-hidden" style={MILITARY_CLIP}>
          ...
          <div className={`${activeTab === "chat" ? "flex" : "hidden"} md:flex flex-1 flex-col min-w-0 border-r border-white/10`}>
            ...
            <form onSubmit={sendMessage} className="p-3 md:p-4 bg-black/40 border-t border-white/5 flex gap-3">
              ...
              <button 
                type="submit"
                disabled={isCooldown}
                className="px-4 md:px-6 py-3 bg-violet-600 hover:bg-violet-500 text-white font-orbitron font-black uppercase italic tracking-widest transition-all shadow-[0_0_15px_rgba(139,92,246,0.3)] disabled:grayscale disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                style={MILITARY_CLIP}
              >
          ...
          <div className={`${activeTab === "users" ? "flex" : "hidden"} md:flex w-full md:w-64 flex-col bg-black/20`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
              {onlineUsers.length === 0 ? (
                <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest text-center mt-8">Nenhum operador detectado</div>
              ) : (
            ...
            <div className="p-4 bg-white/5 border-t border-white/10 mt-auto hidden md:block">
      ...
      <footer className="max-w-6xl mx-auto mt-12 md:mt-20 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-4 opacity-70 grayscale hover:grayscale-0 transition-all relative z-10 font-mono text-[10px] uppercase tracking-[0.5em]">
