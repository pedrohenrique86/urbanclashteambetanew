...
export default function ScrollToTopButton({ scrollableRef, isHomePage }: ScrollToTopButtonProps) {
    ...
    <AnimatePresence>
      {isVisible && !isMobileDrawerOpen && (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={scrollToTop}
          className={`fixed bottom-[14px] md:bottom-12 right-2 md:right-8 ${isHomePage ? 'xl:right-8' : 'xl:right-[300px]'} z-[99999] flex flex-col items-center gap-0 md:gap-1 group cursor-pointer`}
        >
          {/* Tactical Container */}
