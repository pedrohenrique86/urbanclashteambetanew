    ...
    <AnimatePresence>
      {isVisible && !isMobileDrawerOpen && (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={scrollToTop}
          className={`${scrollableRef ? 'absolute' : 'fixed'} bottom-[14px] md:bottom-12 right-4 md:right-8 z-[99999] flex flex-col items-center gap-0 md:gap-1 group cursor-pointer`}
        >
          {/* Tactical Container */}
