...
const changePasswordValidation = [
  ...
  body('newPassword')
    ...
    .withMessage('Nova senha deve ter pelo menos 8 caracteres, incluindo maiúscula, minúscula, número e símbolo')
];
...
router.post('/profile', authenticateToken, async (req, res) => {
  try {
    ...
    if (faction !== 'gangsters' && faction !== 'guardas') {
      return res.status(400).json({ error: 'Facção deve ser: gangsters ou guardas' });
    }
...
router.put('/profile', authenticateToken, updateProfileValidation, async (req, res) => {
  try {
    ...
    const allowedFields = ['faction', 'bio', 'avatar_url', 'level', 'experience_points', 'health', 'energy', 'action_points', 'money', 'victories', 'defeats', 'winning_streak']; // username removido - vem da tabela users
...
router.get('/rankings', async (req, res) => {
  try {
    ...
    console.log('✅ [RANKINGS] Retornando', leaderboardResult.rows.length, 'resultados');
    console.log('✅ [RANKINGS] Primeiro resultado com country:', leaderboardResult.rows[0]?.country);
...
router.put('/:id/profile', authenticateToken, requireOwnership('id'), updateProfileValidation, async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ 
    ...
    if (profileExists.rows.length === 0) {
      ...
      result = await query(`
        ...
        RETURNING *
      `, [
        id, display_name, bio, faction, avatar_url,
        stats.attack, stats.defense, stats.focus, stats.critical_damage,
        stats.critical_chance, stats.intimidation, stats.discipline
      ]);
    } else {
...
router.put('/:id/password', authenticateToken, requireOwnership('id'), changePasswordValidation, async (req, res) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ 
...
router.get('/action-points', authenticateToken, async (req, res) => {
  try {
    const actionPoints = await ActionPointsService.getCurrentActionPoints(req.user.id);
...
router.post('/action-points/consume', authenticateToken, async (req, res) => {
  try {
    ...
    const result = await ActionPointsService.consumeActionPoints(req.user.id, amount);
